...
**Витрина и клиентский слой интернет-магазина цифровых товаров** — подписки, ключи, игровые аккаунты, доступы к сервисам и ИИ: полный пользовательский контур (поиск, карточка, корзина, оплата балансом, заказы, выдача, кабинет, админка). Интерфейс и сценарии соответствуют тому, как сегодня продают цифровику: одна точка входа, понятная сводка по лоту, контроль сделки и поддержка.

В этой поставке **бэкенд и платежи «из коробки» не включены**: для быстрого запуска демо данные и заказы живут в браузере (**`localStorage`**), а вызовы API обрабатывает **`assets/js/core/api.js`** в режиме **`handleDemo`**. Тот же модуль **`API`** умеет работать с **реальным HTTP API** — достаточно задать **`meta name="api-base"`** и отключить демо: клиент готов подключаться к боевому серверу без переписывания экранов.
...
- [Продукт и актуальность](#продукт-и-актуальность)
- [Режимы: демо и продакшен](#режимы-демо-и-продакшен)
- [Зависимости и требования](#зависимости-и-требования)
- [О проекте и данных](#о-проекте-и-данных)
...
## Продукт и актуальность

Рынок **цифровых товаров** (B2C) стабильно растёт: подписки на софт и AI, лицензионные ключи, игровые аккаунты и внутриигровые ценности, доступы к сервисам. Покупатель ожидает **мгновенную выдачу**, прозрачное описание лота и **единый кабинет** со статусом заказа — без этого конверсия падает.

**Снэплот** в интерфейсе заложен как **официальный магазин одного продавца** (не маркетплейс частников): единый каталог, единые правила сделки, эскроу и поддержка как часть сценария. Такая модель актуальна для бренда-дистрибьютора, студии, регионального реселлера или white-label витрины при наличии своего юридического лица и контрактов с правообладателями.

Что уже **можно использовать как основу продукта**:

| Направление | Что даёт репозиторий |
|-------------|----------------------|
| **UX витрины** | Лендинг, каталог с фильтрами, карточка лота, корзина, чекаут |
| **Путь покупателя** | Регистрация/вход, баланс, заказ, страница выдачи |
| **Операции магазина** | Админка: добавление лотов, статусы, обзор заказов |
| **Интеграция** | Единый слой `window.API` — подмена демо на боевой backend |

Юридические документы (оферта, персональные данные, возвраты), платёжный провайдер и серверная часть — **зона ответственности при запуске «в бой»**; клиентский код здесь не заменяет compliance и PCI DSS, но повторяет привычную для отрасли логику экранов.

---

## Режимы: демо и продакшен

| Режим | Когда использовать | Данные и API |
|--------|-------------------|--------------|
| **Демо** (по умолчанию, `snaplot-demo`) | Презентация, портфолио, обучение, прототип без сервера | `localStorage`, `handleDemo` в `api.js`, без исходящих запросов к каталогу |
| **Продакшен** | Рабочий магазин за вашим API | Убрать/не ставить демо-мета, указать **`api-base`**, реализовать сервер под те же контракты `GET/POST`, что разбирает `realFetch` в `api.js` |

Таким образом репозиторий — это не только «учебный макет», а **готовый фронтенд-слой**, который можно наращивать инфраструктурой под свой домен и платежи.
...
**Снэплот** — полный набор экранов официального магазина цифровых товаров: лендинг, каталог, карточка лота, корзина, оформление, заказы, кабинет, кошелёк, выдача по заказу, админка.
...
- В **демо-режиме** покупки, баланс, заказы и пользователи существуют **только в этом браузере** (`localStorage`).
- Вызовы `API.get("listings")` и др. в демо **не уходят в интернет** — их обрабатывает `handleDemo`. При подключении **`api-base`** те же вызовы идут на ваш сервер.
